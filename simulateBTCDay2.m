function [simulated_BTC, aligned_bias, all_trials_arr, trial_blocks_arr] = simulateBTCDay2(subjects,m_prior_init,sd_prior_init,B_struct,sd_likelihood_struct,varargin)
%Based on the parameter values in 'm_prior_init', 'sd_prior_init',
%'B_struct', and 'sd_likelihood_struct', simluate Day 2 of the Bias Time
...
if isempty(varargin)
    block_list.day2 = {'Block1','Block2','Block3','Generalization','Washout','Savings'};
else
    block_list.day2 = varargin{1};
end
ctxt_m = 135;
...
for i = 1:length(fn)
    for k = 1:length(valid_params{i})
        ...
        if rem(k, 50) == 0
            disp([num2str(k), ' simulations done.'])
        end
    end
end     
...
for i = 1:length(fn)
    ...
    xlabel('Day 2 Trial Number')
    ylabel('Inward bias (deg)')
    title({'Simulated Day 2 Bias Time Course Data Using',['Bayesian Parameters Fit From ' fn{i} ' Experimental Data']})
    box('off')
    ylim([-1 15])
end
...
for i = 1:length(fn)
    aligned_bias.(fn{i}) = cell(1,length(block_list.day2));
    all_trials_arr.(fn{i}) = cell(1,length(block_list.day2));
    trial_blocks_arr.(fn{i}) = {};
    for m = 1:length(block_list.day2)
        probe_check = zeros(length(valid_params{i}),20); 
        for j = 1:length(valid_params{i})
            tmp = probe_id{valid_params{i}(j)}(strcmpi(blocks(valid_params{i}(j),probe_id{valid_params{i}(j)}),block_list.day2{m}));
        ...
        for k = 1:size(ur,1)
            if abs(num_standard_probes - sum(ur(k,:) > 0)) > num_probe_dif_thresh
                error(['Large difference in number of probes for different subjects, Block ' block_list.day2{m}])
            end
        end
        ...
        trial_blocks_arr.(fn{i})(length(trial_blocks_arr.(fn{i}))+1:length(trial_blocks_arr.(fn{i}))+num_standard_probes) = block_list.day2(m);
        ...
        for j = 1:length(valid_params{i})
            tmp_probe_trials = probe_id{valid_params{i}(j)}(strcmpi(simulated_BTC.(fn{i}).probe_blocks{j},block_list.day2{m}));
            tmp_bias = simulated_BTC.(fn{i}).bias{j}(strcmpi(simulated_BTC.(fn{i}).probe_blocks{j},block_list.day2{m}));
...
for i = 1:length(fn)
    ...
    xlabel('Day 2 Trial Number')
    ylabel('Inward bias (deg)')
    title({'Mean +/- SE Simulated Day 2 Bias Time Course Data Using',['Bayesian Parameters Fit From ' fn{i} ' Experimental Data']})
end
